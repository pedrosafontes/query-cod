...
jest.mock('../../components/QueryExplorer', () => {
  return jest.fn(() => <div data-testid="query-explorer">Query Explorer Component</div>);
});

jest.mock('../../components/QueryTabs', () => {
  return jest.fn().mockImplementation(({ onSelect, onCreate }) => (
    <div data-testid="query-tabs">
      <button data-testid="create-query" onClick={onCreate}>Create Query</button>
      <button data-testid="select-query" onClick={() => onSelect('1')}>Select Query</button>
    </div>
  ));
});
...
describe('QueriesExplorer Component', () => {
  ...
  beforeEach(() => {
    ...
    (QueriesService.queriesCreate as jest.Mock).mockResolvedValue({ id: 3, text: '' });
  });
