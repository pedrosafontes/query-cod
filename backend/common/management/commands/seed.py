...
class Command(BaseCommand):
    def handle(self, *args: Any, **options: Any) -> None:
        ...
        # Create user
        user = User.objects.create(email='test@querycod.com')
        ...
        ra_lecture_queries = [
            ...
            (
                'Division',
                query('account').project('cname', 'type').divide(query('account').project('type')),
            ),
        ]
        ...
        valid_ra_queries = [
            (
                ...
                query('invoice_line').theta_join(
                    'track', ra.EQ(attribute('invoice_line.track_id'), attribute('track.track_id'))
                ),
            ),
            (
                'Grouped aggregation',
                query('track').grouped_aggregation(
                    ['media_type_id'],
                    [Aggregation(attribute('unit_price'), ra.AggregationFunction.AVG, 'avg_price')],
                ),
            ),
        ...
        ra_semantic_errors = [
            ...
            ('Type mismatch in comparison', query('playlist').select(ra.GT(attribute('name'), 5))),
            (
